...
public final class FlowHost: @unchecked Sendable {
    ...
    public func stop() async -> Int32 {
        // Gone before it is asked — exited, though neither thread has seen it yet — the
        // host ended of its own accord.
        let exited = hasExited()
        lock.withLock {
            if exited { endedOnItsOwn = true }
            stopRequested = true
        }
        notify("host/exit")
    ...
    /// The run has ended — completed or waiting: let the host run out the flow's own timers
    /// and handles, as acpx's process does once its run is over, and wait for it to end,
    /// however long that is. Returns its wait status: `0`, or the code the flow left.
    public func release() async -> Int32 {
        notify("host/release")
        let status = await exitStatus()
        shutdown(socket, SHUT_RDWR)
        return status
    ...
    public var exitedOnItsOwn: Bool {
        let (endedFirst, requested) = lock.withLock { (endedOnItsOwn, stopRequested) }
        return endedFirst || (!requested && hasExited())
    }

    /// Whether the host has exited, reaped or not.
    private func hasExited() -> Bool {
        var info = siginfo_t()
        while waitid(P_PID, id_t(pid), &info, WEXITED | WNOHANG | WNOWAIT) != 0 {
            // No such child any more: the waiter has reaped it.
