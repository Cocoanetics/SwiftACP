...
/// A prompt's answer or failure, and a request of the agent's, are announced in the order
/// they crossed the wire (#124, #106), as acpx's formatter sees every message: an update
/// the agent sends straight after its answer comes after it, and a request it makes right
/// before comes before — however late the prompt's call resumes, or the request is served.
struct PromptAnswerOrderTests {
    /// Plays the agent on `agent`: answers the handshake and a new session, and a prompt
    /// with its answer — just after asking to read a file, when `asking`; its internal
    /// error, when `failing` — and, at once after that, one more reply chunk.
    private static func playAgent(on agent: LoopbackTransport, asking: Bool = false, failing: Bool = false)
        async throws {
        for try await message in agent.makeInboundStream() {
            ...
            case "session/prompt":
                ...
                try agent.send(failing
                    ? .errorResponse(id: request.id, error: JSONRPCErrorBody(code: -32603, message: "Internal error"))
                    : .response(id: request.id, result: answer))
    ...
    /// An update the agent sends straight after failing the prompt comes after the failure,
    /// where acpx's formatter shows the error.
    @Test(.timeLimit(.minutes(1)))
    func anUpdateRightAfterAFailureComesAfterIt() async throws {
        let (clientEnd, agentEnd) = LoopbackTransport.pair()
        let agent = Task { try await Self.playAgent(on: agentEnd, failing: true) }
        defer { agent.cancel() }
        let client = ACPAgentConnection(transport: clientEnd, handlers: .standard(permission: .approveAll))
        await client.start()
        _ = try await client.initialize(capabilities: ClientCapabilities(), clientInfo: .acpx)
        ...
        let order = Task { () -> [String] in
            ...
            for await event in stream {
                switch event {
                case .promptFailed(_, let error): seen.append("failed \(error.code)")
                case .update(let note):
                    guard case .agentMessageChunk(let block) = note.update, block.text == "late" else { continue }
                    seen.append("late")
                    late.finish()
        ...
        await #expect(throws: JSONRPCErrorBody.self) {
            try await client.prompt(PromptRequest(sessionId: session.sessionId, prompt: [.text("hi")]))
        }
        for await _ in lateSeen { break }
        await client.endSubscription(subscription)

        #expect(await order.value == ["failed -32603", "late"])
        await client.close()
    }
