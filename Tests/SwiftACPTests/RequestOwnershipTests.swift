...
struct RequestOwnershipTests {
    ...
    private static func playAgent(on agent: LoopbackTransport, answers: Answers, script: Script) async throws {
    ...
    private static func answer(_ prompt: JSONRPCID, _ stopReason: StopReason = .endTurn) throws -> JSONRPCMessage {
        .response(id: prompt, result: try JSONValue(encoding: PromptResponse(stopReason: stopReason)))
    }

    private static let permission: JSONValue = .object([
        "sessionId": .string("s"),
