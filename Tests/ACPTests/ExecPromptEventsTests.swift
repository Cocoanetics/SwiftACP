...
struct ExecPromptEventsTests {
    ...
    /// An update is read before its effect is noted. One read during the pause whose
    /// effect is noted only as the pause's end hands it on still calls the retry off:
    /// the effects are looked at again once it has been handed on.
    @Test(.enabled(if: mockPythonAvailable), .timeLimit(.minutes(1)))
    func anUpdateWhoseEffectIsNotedLateStillCallsTheRetryOff() async throws {
        let agent = try await Self.launchFixture(mode: "fail-then-update")
        let written = Written()
        let renderer = OutputRenderer(
            options: RenderOptions(format: .text), out: { written.append($0) }, err: { _ in }, color: false)
        ...
        let prompts = Counter()
        let (released, release) = AsyncStream<Void>.makeStream()
        await agent.connection.setWireMessageObserver { direction, message in
            if direction == .outbound, case .request(let request) = message, request.method == "session/prompt" {
                prompts.add()
            }
            if !held.hold(message) { sideEffects.observe(direction, message) }
        }
        // The held update's effect is noted as it is handled, once the pause's end waits.
        await agent.connection.setBeforeHandlingUpdate {
            for await _ in released { break }
            held.release { sideEffects.observe(.inbound, $0) }
        }
        await agent.connection.setOnUpdateWait { _ in release.finish() }
        do {
            let response = try await agent.connection.newSession(
    ...
    /// The first `session/update` read, kept from the side effects until released.
    private final class HeldUpdate: @unchecked Sendable {
        private let lock = NSLock()
        private var taken = false
        private var message: JSONRPCMessage?

        /// Keep `message` if it is the first update: `true` when it was kept.
        func hold(_ message: JSONRPCMessage) -> Bool {
            guard case .notification(let note) = message, note.method == "session/update" else { return false }
        ...
        /// Hand the kept update to `body`, once.
        func release(_ body: (JSONRPCMessage) -> Void) {
            let kept: JSONRPCMessage? = lock.withLock {
                defer { message = nil }
                return message
            }
            if let kept { body(kept) }
        }
    }
